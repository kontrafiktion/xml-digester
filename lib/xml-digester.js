...
;(function (xml_digester) {
  ...
  OrderedElementsHandler.prototype.onopentag = function(node, digester) {
    ...
    if ( this.name_property ) {
      ...
      new_object._name = node.name;
    }


    if ( has_properties(parent_object) ) {
      console.warn("Ordered Element Container must not have attributes: ");
      console.warn(parent_object);
    }    
    ...
    // TODO: refactor: the following is from the default handler
    digester.current_object = new_object;

  }

  // TODO: make elements intelligent? instead of putting everything in stateless handlers?

  OrderedElementsHandler.prototype.onclosetag = function(node_name, digester) {
    var parent_object = digester.object_stack.pop();

    if ( ! Array.isArray(parent_object) ) {
      log.error("internal error: object should be Array")
    }

    // TODO: refactor: this block is from the default handler
    ...
    digester.sax.onopentag = function (node) {
      ...
      if ( digester.handler ) {
        digester.handler.forEach(function(elem) {
          ...
          if ( match_stack(elem.path, digester.xpath_stack) ) {
